    ...
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            routerBuilder.MapRoute("{conntroller}/{action}/{id?}", async context =>
            {
                ...
                await context.Response.WriteAsync("Запит з 3 сигментами, {conntroller}/{action}/{?id}");
            });

            routerBuilder.MapRoute("{lang}/{conntroller}/{action}/{id?}", async context =>
            {
                context.Response.ContentType = "text/html; charset=utf-8";
                await context.Response.WriteAsync("Запит з 4 сигментами, {lang}/{conntroller}/{action}/{?id}");
            });
        ...
        private async Task AddToSeesion(HttpContext context)
        {
            try
            {
                ...
                context.Response.ContentType = "text/html; charset=utf-8";
                await context.Response.WriteAsync("Додавання в сесію пройшло успішно !");
            }
        ...
        private async Task SetCooke(HttpContext context)
        {
            try
            {
                ...
                context.Response.ContentType = "text/html; charset=utf-8";
                await context.Response.WriteAsync("Додавання в кукі пройшло успішно !");
            }
            catch (Exception erorr)
            {
                context.Response.ContentType = "text/html; charset=utf-8";
